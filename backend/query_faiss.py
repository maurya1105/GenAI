...
Your task is to interpret metadata and configuration instructions to accurately read, understand or update the database tables by generating field values. Follow these guidelines:
...
1) For Bill Cycle:
    ...
    -All rows to be added in CI_BILL_CYC_SCH must be insert query format and all rows values must be generated by you. Do not provide logic code for it. Provide direct INSERT query with multiple rows.
    -Only 1 row needs to be added in CI_BILL_CYC and CI_BILL_CYC_L since only one primary key BILL_CYC_CD='XX'
...
ALWAYS answer in SQL insert format for all the tables (including parent, and all its children) that need to be updated.
...
if __name__ == "__main__":
    ...
    print("Enter: \n pf0 for C2M info prompt template \n pf1 for DB info prompt template \n pf2 for DB config help prompt template \n\nDefault prompt template is C2M info")
    while(True):
