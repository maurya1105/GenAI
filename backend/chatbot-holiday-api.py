...
def work_cal_country_extract(query):
...
def query(query_text, prompt_index, history=None, holiday_list=None):    
    ...
    try:
        if holiday_list:
            for chunk in chain.stream({"question":query_text, "holiday_list":holiday_list, "history":history}):
                print(chunk, end="", flush=True)
                response_chunks.append(chunk)
        else:
    ...
    while(True):
        ...
        try:
            if ind==3:
                ...
                logging.debug(f"Accessed Gsheets for country and state address formats")
...
"""         
                ...
                logging.info(f"Query matched with NLP Classifier for Work Calendar object.") 

                #routing to prompt for work calendar -- API needed for holidays
                country, year = work_cal_country_extract(inp)
                print("Country=", country, "\nYear=", year)

                if country!=None:
                    holiday_list=get_or_update_work_cal(country, year)
                    logging.info(f"Holiday list acquired for {country} in {year}") 
                    # print(holiday_list)
                  
                    # response = query(inp, 5, str(history), holiday_list)
                    response = query(inp, 6, str(history))
                    history.append({"role": "assistant", "content": response[0], "sources": " "})
                    response = query(inp, 7, str(history), holiday_list)
                    history.append({"role": "assistant", "content": response[0], "sources": " "})
                else:
