...
def main():
    ...
    parser.add_argument("--add-new", action="store_true", help="Add new documents to the database.")
    parser.add_argument("--add-new-instruct", action="store_true", help="Add only new instruction documents to the database.")
    parser.add_argument("--add-new-tbl", action="store_true", help="Add only new tbl to the database.")
    ...
    if args.reset or args.add_new:
        ...
        try:
            ...
            with open('data/schema_description.txt', 'r', encoding='utf-8') as f:
                text = f.read()
                metadata = {"source": 'data/schema_description.txt'}
                document = Document(page_content=text, metadata=metadata)
            chunks = split_documents(document)
            add_tbl_to_faiss(chunks, args.add_new)

        except:
            print("Schema data not found")

    elif args.add_new_instruct:
        extract_files()
        documents, images = load_documents()
        if images:
            save_images(images)
...
def load_documents():
    documents = []
    images = []
    supported_extensions = (".pdf", ".csv", ".txt", ".html", ".java", ".xml", ".properties", ".md" ,".class", ".ppt", ".pptx", ".jsp", ".jspx", ".js", ".json", ".doc", ".docx")
