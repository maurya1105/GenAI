...
def classify_and_extract(query):
    prompt = (
        f"Classify this query into one of the following categories: {', '.join(menu_items.keys())}. "
        f"If it asks a functional question, classify it as 'C2M info', if it asks a general question about the database, it should be classified as 'DB info'. Else, if any query asks to 'configure' or 'create' or give a particular object configuration then that query must be classified as 'DB config help'. If the object to be configure is country -- classify it as 'Country config'"
        f"If it belongs to 'Country config', extract the country name. "
        f"Return a JSON object with 'menu_item' and 'details' keys.\n\nQuery: {query}"
    )
    ...
    try:
        # Look for the first and last curly braces to isolate the JSON
        ...
        if json_start != -1 and json_end != -1:
            ...
            menu_item = response_json.get("menu_item", "").strip()
            if menu_item.lower()=='country config':
                details = response_json.get("details", {}).get("country_name", "").strip()
            else:
                details = response_json.get("details", "").strip()
            return menu_item, details
        else:
...
if __name__ == "__main__":
    ...
    while(True):
        ...
        menu_item, details = classify_and_extract(inp)
        
        if menu_item:
            ind = menu_items.get(menu_item, 0)
            print(f"Selected menu index: {ind} ({menu_item})")
        ...
        try:
            if ind==3:
                insert_ci_country, insert_ci_country_l = get_address_format_by_country_name_sheet(details)
...
"""         
                states_country = get_states_by_country_name_api(details)
